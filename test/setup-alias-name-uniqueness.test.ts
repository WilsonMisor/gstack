...
// This suite deliberately forces IS_WINDOWS=0 so it can exercise the POSIX
// symlink install shape and prove alias rewriting never writes through a
// symlink. On native Windows that is the wrong install path to test: gstack's
// real setup uses IS_WINDOWS=1 and copy semantics there, with separate Windows
// regression coverage. Running this POSIX-only fixture under Git Bash on
// Windows can wedge `ln -snf`/filesystem traversal despite native-symlink
// privileges being available, so keep the Unix contract on Unix hosts.
if (process.platform === 'win32') {
  describe('alias installs are rewritten copies (#2511, #2201)', () => {
    test.skip('POSIX alias-install semantics are covered on macOS/Linux; Windows uses the copy path', () => {});
  });
} else {
  beforeAll(() => {
    const installOnce = [
