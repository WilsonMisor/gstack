...
(project intake, planning, production implementation, review, QA, shipping,
debugging, docs, security, design). For browser/QA and dogfooding it points you
at /browse. Use when you invoke gstack without a specific skill, or ask "which
gstack skill fits this?".
...
## Canonical PREOS production pipeline

For full production work, preserve this external Stage 0-10 pipeline exactly:

0. `/project-init` — ingest project/source package and establish the Project Contract.
1. `/office-hours` — product/problem interrogation.
2. `/plan-ceo-review` — product and strategic scope review.
3. `/plan-eng-review` — architecture and engineering review.
4. `/autoplan` — consolidated planning and final human approval.
5. `/production-implement` — production code implementation under PREOS + the immutable 75-control baseline.
6. `/review` — independent engineering/code review.
7. `/qa` — independent behavioural/browser QA.
8. `/ship` — release readiness and plan-completion audit.
9. `/land-and-deploy` — controlled merge and deployment.
10. `/canary` — production verification.

PREOS is cross-cutting intelligence inside this pipeline. Do NOT create or require extra
mandatory `/risk-model`, `/architecture-economics`, `/production-plan`, or
`/production-learn` stages. Existing specialist domain skills remain independent and are
bound through the Project Contract rather than copied into gstack.

**Routing rules — when you see these patterns, INVOKE the skill via the Skill tool:**
- User supplies a project package/repo/folder/ZIP/source-of-truth documents, asks to initialize a production project, establish project truth, or begin the full Stage 0-10 pipeline → invoke `/project-init`
- User asks to implement/build/code an already approved plan under production controls, or says the plan is approved and ready to build → invoke `/production-implement`
- User says "start from the last session", "resume where I left off", "where was I", or asks to restore saved work → invoke `/context-restore`; if Stage 5 production state exists, `/production-implement` must then reconcile persistent state + Git before edits
